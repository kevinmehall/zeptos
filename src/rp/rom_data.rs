// Based on rp-hal under MIT or Apache-2.0
// https://github.com/rp-rs/rp-hal/blob/e87e2acc233a9e3b54a9287dc2a7285db1969cad/rp2040-hal/src/rom_data.rs
...
#[no_mangle]
extern "C" fn __popcountdi2(x: u32) -> u32 {
    popcount32(x)
}
...
#[no_mangle]
extern "C" fn __clzdi2(x: u32) -> u32 {
    clz32(x)
}
...
#[no_mangle]
extern "C" fn __ctzdi2(x: u32) -> u32 {
    ctz32(x)
}
...
#[no_mangle]
unsafe extern "aapcs" fn __aeabi_memset8(dest: *mut u8, n: usize, c: i32) -> () {
    // Different argument order
    memset4(dest as *mut u32, c as u8, n as u32);
}
...
#[no_mangle]
unsafe extern "aapcs" fn __aeabi_memclr8(dest: *mut u8, n: usize) -> () {
    memset4(dest as *mut u32, 0, n as u32);
}
...
#[no_mangle]
unsafe extern "aapcs" fn __aeabi_memcpy8(dest: *mut u8, src: *const u8, n: usize) -> () {
    memcpy44(dest as *mut u32, src as *const u32, n as u32);
}
            ...
            $(
                declare_rom_function! {
                    ...
                    fn $name( $( $aname : $aty ),* ) -> $ret {
                        let table: *const usize = soft_float_table();
                        unsafe {
        ...
        (
            ...
            $(
                declare_rom_function! {
                    $(#[$outer])*
                    fn $name( $( $aname : $aty ),* ) -> $ret {
                        let table: *const usize = soft_double_table();
                        unsafe {
                            // This is the entry in the table. Our offset is given as a
                            // byte offset, but we want the table index (each pointer in
                            // the table is 4 bytes long)
